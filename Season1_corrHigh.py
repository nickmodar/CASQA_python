################################################################################
################################################################################
################### ~ Season 1: Correlation Figure (high) ~ #####################
################################################################################
################################################################################
import numpy as np
...
lat = data1IN['lat'][:]
lon = data1IN['lon'][:]
corrHI_High = season1_in['spatial_correlations_HIhigh'][:]
...
corrHI_low_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), corrHI_High.flatten(), np.nan)
