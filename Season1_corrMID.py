################################################################################
################################################################################
################### ~ Season 1: Correlation Figure (mid) ~ #####################
################################################################################
################################################################################
import numpy as np
...
lat = data1IN['lat'][:]
lon = data1IN['lon'][:]
corrHI_mid = season1_in['spatial_correlations_HImid'][:]
...
corrHI_low_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), corrHI_mid.flatten(), np.nan)
