################################################################################
################################################################################
################## ~ Season 1: Haines Index Figure (mid) ~ #####################
################################################################################
################################################################################
import numpy as np
...
season1_Fin = 'C:/Users/modarn/Documents/CASQA/data/season1982_1983.nc'
season1_in = nc.Dataset(season1_Fin)
data1IN = nc.Dataset(fn1)# load in original nc file for lat,lon (and time maybe) dimensions.
lat = data1IN['lat'][:]
lon = data1IN['lon'][:]
HI_mid_in = season1_in['HI_mid'][:]
HI_mid = np.mean(HI_mid_in, axis=0)
...
HI_mid_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), HI_mid.flatten(), np.nan)
HI_mid_masked = HI_mid_masked.reshape(lon_mesh.shape)
...
contour = plt.contourf(lon_mesh, lat_mesh, HI_mid_masked,
                       levels= 6, extend='both', cmap=cmap.reversed('RdYlGn'))#, vmin=0, vmax=6)
