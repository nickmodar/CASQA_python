...
lat = data1IN['lat'][:]
lon = data1IN['lon'][:]
HI_high_in = season1_in['HI_high'][:]
HI_high = np.mean(HI_high_in, axis=0)
...
HI_high_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), HI_high.flatten(), np.nan)
HI_high_masked = HI_high_masked.reshape(lon_mesh.shape)
...
contour = plt.contourf(lon_mesh, lat_mesh, HI_high_masked, levels= 10, extend='both', cmap=cmap.reversed('RdYlGn'))
