################################################################################
################################################################################
################### ~ Season 2: Correlation Figure (high) ~ #####################
################################################################################
################################################################################
import numpy as np
...
corrHI_high_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), corrHI_high.flatten(), np.nan)
corrHI_high_masked = corrHI_high_masked.reshape(lon_mesh.shape)

# Create contour plot for the masked data with specified vmin and vmax
cmap = plt.get_cmap('bwr')
contour = plt.contourf(lon_mesh, lat_mesh, corrHI_high_masked, cmap=cmap, levels=np.linspace(-1, 1, 21),
