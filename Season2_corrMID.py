################################################################################
################################################################################
################### ~ Season 2: Correlation Figure (mid) ~ #####################
################################################################################
################################################################################
import numpy as np
...
corrHI_mid_masked = np.where(mask.contains(gpd.points_from_xy(lon_mesh.flatten(), lat_mesh.flatten())), corrHI_mid.flatten(), np.nan)
corrHI_mid_masked = corrHI_mid_masked.reshape(lon_mesh.shape)

# Create contour plot for the masked data with specified vmin and vmax
cmap = plt.get_cmap('bwr')
contour = plt.contourf(lon_mesh, lat_mesh, corrHI_mid_masked, cmap=cmap, levels=np.linspace(-1, 1, 21),
