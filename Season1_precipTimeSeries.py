...
ax = plt.gca()
ax.plot(dates_list, daily_sum_precip, linestyle='-', linewidth='5', color='darkorange')

# Y-AXIS
min_value = np.min(daily_sum_precip)
max_value = np.max(daily_sum_precip)
y_ticks = np.linspace(min_value, max_value+0.8711681365966797,num=12)
ax.set_ylim(0, max_value + 0.8711681365966797)
plt.yticks(y_ticks)
...
locs = list(ax.get_xticks())+ [mdates.date2num(datetime(1983,4,1))]
...
ax.set_xlabel(r'Event 1: California Wet Season' + '\n' + r'November 1, 1982 - April 1, 1983', fontsize=24)
